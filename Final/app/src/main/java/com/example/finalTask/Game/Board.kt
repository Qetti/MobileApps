...
class Board {
    ...
    val isGameOver: Boolean
        get() = hasMishaWon() || hasBidzinaWon() || availableCells.isEmpty()


    //These functions are checking
    //Weather the misha or bidzina has won or not
    fun hasMishaWon(): Boolean {
    ...
    fun minimax(depth: Int, player: String): Int {
        if (hasMishaWon()) return +1
        if (hasBidzinaWon()) return -1
        ...
        for (i in availableCells.indices) {
            ...
            if (player == COMPUTER) {
                ...
                val currentScore = minimax(depth + 1,
                    PLAYER
                )
